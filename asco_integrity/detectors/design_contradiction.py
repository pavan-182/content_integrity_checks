"""Sentence-local V1 design checks; mixed components are suppressed conservatively."""
...
SYSTEM_PROMPT = (
    ...
    "refer to the same study component and form a genuine wording contradiction. Do not judge "
...
PATTERNS: tuple[PatternSpec, ...] = (
    ...
    _pattern("allocation", "randomized", r"(?<!non-)(?<!not )\brandomi[sz]ed\b"),
...
FUTURE_DESIGN_RE = re.compile(
    r"\b(?:future|further|additional)?\s*(?:prospective|randomi[sz]ed|interventional|"
    r"placebo[- ]controlled)\s+(?:studies|trials|research|validation)\s+"
    r"(?:are\s+)?(?:needed|required|warranted|recommended|planned)\b|"
    r"\b(?:prospective|randomi[sz]ed) (?:studies|trials)\s+should be conducted\b",
    re.IGNORECASE,
)
...
RULES: tuple[RuleSpec, ...] = (
    ...
    (("design_name", "cohort"), ("allocation", "randomized"), "cohort_vs_randomized_allocation", "medium"),
...
PROSPECTIVE_COLLECTION_RETROSPECTIVE_ANALYSIS_RE = re.compile(
    ...
    r"\b(?:retrospective analysis|analy[sz]ed retrospectively)\b|"
    r"\b(?:retrospective analysis|analy[sz]ed retrospectively)\b.*"
    r"\b(?:prospective(?:ly)? (?:data )?collect(?:ion|ed)|"
    r"(?:data )?(?:were )?collected prospectively)\b",
    re.IGNORECASE | re.DOTALL,
)
DISCOVERY_VALIDATION_RE = re.compile(
    r"\bdiscovery cohort\b.*\bvalidation cohort\b|\bvalidation cohort\b.*\bdiscovery cohort\b",
    re.IGNORECASE | re.DOTALL,
)
...
def _component(sentence: str) -> str:
    ...
    markers = (
        ...
        ("historical_control", "historical control"),
        ("external_control", "external control"),
        ("part_a", "part a"),
        ("part_b", "part b"),
    )
    return next((name for name, marker in markers if marker in normalized), "main")


def _same_component(left: StudyDesignClaim, right: StudyDesignClaim) -> bool:
    left_component, right_component = _component(left.sentence), _component(right.sentence)
    return left_component == right_component or left_component == right_component == "main"
...
def _suppressed(
    ...
    if not _same_component(left, right):
        return True
    if PRIOR_TRIAL_ANALYSIS_RE.search(record_text):
        return True
    if DERIVED_ANALYSIS_RE.search(record_text):
        return True
    if DISCOVERY_VALIDATION_RE.search(record_text):
        return True
    if contradiction_type == "prospective_vs_retrospective":
        return bool(PROSPECTIVE_COLLECTION_RETROSPECTIVE_ANALYSIS_RE.search(record_text))
    if contradiction_type == "open_label_vs_blinded":
        return bool(OPEN_LABEL_EXTENSION_RE.search(record_text))
    if contradiction_type == "phase_2_vs_phase_3":
        return bool(SEAMLESS_PHASE_RE.search(record_text) or SEPARATE_PHASE_RE.search(record_text))
    if contradiction_type == "single_arm_vs_controlled_arms":
        return bool(HISTORICAL_CONTROL_RE.search(record_text))
    return False
