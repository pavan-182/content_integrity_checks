...
class ReportingReconciliationTests(unittest.TestCase):
    ...
    def test_authorship_checks_come_from_frontend_json(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            dictionary = _write(root, "dict.csv", "Fingerprint - Tortured Phrase,Expected Text,Nb Retrieved Papers\n")
            # Own fixture rather than the repo's outputs/frontend_run/final_json.json: that file
            # is regenerated by real runs, so asserting on its wording made this test fail
            # whenever the frontend report changed.
            authorship = _write(
                root,
                "authorship.json",
                json.dumps(
                    {
                        "10.1002/cam4.2839": {
                            "abstract_id": "AUTH-1",
                            "checks": [
                                {"check_name": name, "result": {"level": level, "comment": comment}}
                                for name, level, comment in (
                                    ("submission_volume", "HIGH", "Jane Doe submitted 7 times."),
                                    ("author_count_deviation", "LOW", "Author count is typical."),
                                    ("affiliance_relevance", "HIGH", "Affiliation does not match the topic."),
                                    ("author_network", "HIGH", "Authors co-submit repeatedly."),
                                    ("retraction_history", "HIGH", "One prior retraction."),
                                )
                            ],
                        }
                    }
            ...
            result = run_default_pipeline(
                input_dir, dictionary, root / "output", authorship_json_path=authorship
            )
